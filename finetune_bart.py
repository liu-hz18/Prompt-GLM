...
class MultipleChoiceDataset(Dataset):
    ...
    def __getitem__(self, index):
        return {
            ...
            "gt": self.gts[index],
            "label": self.choices.index(self.gts[index])
        }
...
def parse_args():
    ...
    # model
    parser.add_argument("--backbone", type=str, default="facebook/bart-large")
    # fine-tuning options
...
def finetune_cls_task(dataset_name: str, prompt_name: str, choices: list=["Yes", "No"]):
    ...
    for choice_str in choices:
        choice_id = tokenizer(choice_str).input_ids[1]
    ...
    if not args.test_only:
        for e in range(1, args.epoch+1):
            ...
            for i, sample in tbar:
                ...
                logits = model(input_ids=input_ids, attention_mask=attention_mask).logits # [B, L, E]
                masked_index = (input_ids == tokenizer.mask_token_id).nonzero()
                choice_logits = []
                for idx in masked_index:
                    choice_logits.append(logits[idx[0], idx[1], choice_ids]) # [B, C]
                choice_logits = torch.stack(choice_logits)
                loss = F.cross_entropy(choice_logits, sample["label"].cuda())
            ...
            with torch.no_grad():
                for i, sample in tqdm(enumerate(cls_valid_dataloader, start=1), desc="valid", total=len(cls_valid_dataloader)):
                    ...
                    input_ids = encodings.input_ids
                    attention_mask = encodings.attention_mask
                    logits = model(input_ids=input_ids, attention_mask=attention_mask).logits # [B, L, C]
                    masked_index = (input_ids == tokenizer.mask_token_id).nonzero()
                    choice_logits = []
                    for idx in masked_index:
                        choice_logits.append(logits[idx[0], idx[1], choice_ids]) # [B, C]
    ...
    with torch.no_grad():
        for i, sample in tqdm(enumerate(cls_test_dataloader, start=1), desc="test", total=len(cls_test_dataloader)):
            ...
            loss = F.cross_entropy(choice_logits, sample["label"].cuda())
            test_loss += loss.item()
            test_preds.extend(torch.argmax(choice_logits, dim=-1).cpu().numpy().tolist())
            test_gts.extend(np.array(sample["label"]).tolist())
...
if __name__ == "__main__":
    ...
    logger.info(config)
    tokenizer = BartTokenizer.from_pretrained(args.backbone, cache_dir=MODEL_CACHE_DIR)
    model = BartForConditionalGeneration.from_pretrained(args.backbone, cache_dir=MODEL_CACHE_DIR).to(device)
